            ...
            for c in range(0, int(l['qty'])):
                ...
                if price_display:
                    ...
                    if tax_included and p.taxes_id:
                        tax_value = p.taxes_id._compute_amount(round(p.lst_price,2), p.currency_id, c, p)
                        tax_price_value = round(p.lst_price, 2) + tax_value
                        if tax_round:
                            tax_round_value = str(round(tax_price_value))
                            price_value = tax_round_value
