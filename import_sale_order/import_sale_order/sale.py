...
class sale_order(models.Model):
	...
	cn_no = fields.Char('CN#')

class gen_sale(models.TransientModel):
	_name = "gen.sale"

	file = fields.Binary('File')
	sequence_opt = fields.Selection([('custom', 'Use Excel/CSV Sequence Number'), ('system', 'Use System Default Sequence Number')], string='Sequence Option',default='system')
	...
	def make_sale(self, values):
		...
		if sale_search:
			...
			warehouse_id = self.find_warehouse(values.get('warehouse'))
			analytic_account_id = self.find_analytic_account(values.get('analytic_account'))
	...
	def find_analytic_account(self, name):
		analytic_account_obj = self.env['account.analytic.account']
		analytic_account_search = analytic_account_obj.search([('name', '=', name)])
		if analytic_account_search:
			return analytic_account_search
		else:
			analytic_account_id = analytic_account_obj.create({
				'name' : name})
	...
	def import_sale(self):
		...
		if self.import_option == 'csv':
			keys = ['order', 'customer', 'pricelist','product', 'quantity', 'uom', 'description', 'price','user','tax','date','discount','customer_ref','warehouse_id']
			try:
